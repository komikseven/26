...
export function Reader({ id }: { id: number }) {
  ...
  // ── Intercept tombol back device → selalu ke detail komik ───────────────
  useEffect(() => {
    if (!chapter?.seriesSlug) return
    const detailUrl = `/komik/${chapter.seriesSlug}`
    // Tambah state dummy agar ada history entry untuk dicegat
    window.history.pushState({ readerGuard: true }, "", window.location.href)
    const handlePopState = () => {
      router.replace(detailUrl)
    }
    window.addEventListener("popstate", handlePopState)
    return () => window.removeEventListener("popstate", handlePopState)
  }, [chapter?.seriesSlug, router])
    ...
    <div className="relative">
      ...
      >
        <div className="bg-black/75 backdrop-blur-sm flex items-center gap-2 px-3 py-1.5">
          <Link
            href={chapter.seriesSlug ? `/komik/${chapter.seriesSlug}` : `/`}
